...
const kategoriOptions = [
  { value: 'APBDesa', label: 'APBDesa' },
  { value: 'Dana_Desa', label: 'Dana Desa' },
];

export const AnggaranForm: React.FC<AnggaranFormProps> = ({ onSuccess }) => {
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({
    judul: '',
    deskripsi: '',
    kategori: '' as 'APBDesa' | 'Dana_Desa' | '',
  ...
  const uploadImage = async (file: File): Promise<string> => {
    ...
    const fileName = `anggaran-${Date.now()}.${fileExt}`;
    const filePath = `anggaran/${fileName}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isAdmin) {
      toast({
        title: "Akses Ditolak",
        description: "Hanya admin yang dapat menambahkan data anggaran",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description: "Sedang menyimpan data anggaran",
      });
        ...
        .insert([
          {
            ...
            kategori: formData.kategori as 'APBDesa' | 'Dana_Desa',
      ...
      toast({
        ...
        description: `Data anggaran "${formData.judul}" telah disimpan`,
      });
